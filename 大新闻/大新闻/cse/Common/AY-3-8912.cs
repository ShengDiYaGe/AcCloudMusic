...
processors that this IC was originally part of. The BC1 and BDIR pins are used 
to control the IC as follows:
...
In addition to using BC1 and BDIR, A8 should be taken high to enable the chip. 
IOA0-IOA7 are the input/output pins (the AY-3-8910 has an additional set 
marked IOB0-IOB7). The IC does not contain an internal oscillator - the clock 
input to the CLK pin should fall between 1MHz-2MHz, so a typical Z80 system's 
...
The volume registers (8, 9 and 10) contain a 4-bit setting but if bit 5 is set 
then that channel uses the envelope defined by register 13 and ignores its 
volume setting. 
...
Pitch values
The course and fine pitch registers for each channel are used in the following 
fashion (assuming channel A):
...
written to these, the lower the pitch. For example, if a frequency of 1KHz was 
required and the IC's clock frequency was 1MHz, a total division rate of 1000 
would be needed. The sound generator itself divides by 16 so the course and 
fine pitch registers must provide a further division by 62.5 (due to the fact 
that 1000/16 is 62.5). A division rate of 62 or 63 will be accurate enough, 
since the registers can only store whole numbers. Therefore, 62 or 63 would be 
written to register 0 and 0 would be written to register 1.
...
Atari ST 
Sega Master System 
There are also a few AY emulators available at the time of writing:

Z80 - The Spectrum emulator by Gerton Lunter for PCs and compatibles which 
utilises the Adlib sound generator present on most Soundblaster-compatible 
cards. This package fully emulates the Spectrum 48/128 (apart from 4-bit 
sample output) and is well worth looking at. 

DeliAY - by Patrik Rak for the Commodore Amiga. This comes as an add-on for 
"DeliTracker" (a music player) and emulates a Z80 and AY-3-8912 (in a 
Spectrum-like fashion) giving very accurate results. A growing number of 
Spectrum tunes are being gathered together to run under this player. 
YM Player and converter tool (from Z80 .out files). Lots of tunes at the 
site too. 
Port Specifics
The control (register select) and data output ports are as follows (in decimal):

Machine Port Addresses Machine Name Control Data 
...
This text was entered by Alastair Booker on 4th April 1995. Please feel 
free to contact me regarding anything contained in this document. I have 
produced a generic AY-3-8912 interface circuit for Z80-based systems and 
will make this available to anyone who requires it.

email: ali@squark.demon.co.uk


--------------------------------------------------------------------------------

Technical Index PD Groups page Intro page 

File last updated: 16 June 1998 (HTML converted by John Garner)
Author: Alastair Booker, credits also to Alvin Albrecht for TS2068 port info
